...
def make_board(solution):
    board = " ".join(["".join(["-" for _ in split]) for split in solution.split()])
    return board
...
def main():
    ...
    while True:
        spin = None
        turn = input(
            "Would you like to [s]pin, [b]uy a  vowel, or solve the [p]uzzle? "
        )
        if turn.lower() == "s":
            ...
            if is_correct is True:
                print(
                    f"You got it right! It took you a total of {turns} turns and you had ${bank} remaining!"
                )
                break
