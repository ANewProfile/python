...
def send():
    ...
    if readable.lower() == 'exit':
        conn.sendall(readable.encode())
        SENT_EXIT = True
        on_exit()
    else:
        msg = f'{time.time()}, from server: {readable}'
        ...
        c.execute('INSERT INTO messages VALUES (?,?,?)', (str(time.time()), 'server', readable))
        sql_conn.commit()
...
def on_exit():
    global APP_OPEN, SQL_OPEN, CONNECTED, SENT_EXIT, root, sql_conn, s, conn
    if APP_OPEN:
        root.destroy()
        APP_OPEN = False
