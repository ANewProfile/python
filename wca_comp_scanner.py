...
html = get_data('https://www.worldcubeassociation.org/competitions?region=all&search=Mass&state=present&year=all+years&from_date=&to_date=&delegate=&show_registration_status=on&display=list')
...
print(termcolor.colored(num_comps.string, 'blue', attrs=['bold']))
print('----------------------------------------------------------\n')
max_name_length = max(len(item[1]) for item in comps_locs)
